...
var question = require("../questions/charlie");
...
App = (function() {
    ...
    function App() {
        ...
        this.onClickDownload = bind(this.onClickDownload, this);
        this.onClickReference = bind(this.onClickReference, this);
        this.onClickInstructions = bind(this.onClickInstructions, this);
        this.deactivatePowerMode = bind(this.deactivatePowerMode, this);
        this.activatePowerMode = bind(this.activatePowerMode, this);
        this.drawParticles = bind(this.drawParticles, this);
        ...
        this.$finish.on("click", this.onClickDownload);
    ...
    App.prototype.getCursorPosition = function() {
        var left, ref, top;
        ref = this.editor.renderer.$cursorLayer.getPixelPosition(), left = ref.left, top = ref.top;
    ...
    App.prototype.spawnParticles = function(type) {
        var color, numParticles, ref, x, y;
        if (!this.powerMode) {
            return;
        }
        ref = this.getCursorPosition(), x = ref.x, y = ref.y;
    ...
    App.prototype.drawParticles = function(timeDelta) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
            ...
            results.push(this.canvasContext.fillRect(Math.round(particle.x - this.PARTICLE_SIZE / 2), Math.round(particle.y - this.PARTICLE_SIZE / 2), this.PARTICLE_SIZE, this.PARTICLE_SIZE));
        }
        return results;
    };
    ...
    App.prototype.onClickDownload = function() {
        var $a;
        $a = $("<a>").attr({
